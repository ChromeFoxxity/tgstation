...
/datum/controller/subsystem/ticker/proc/send_news_report()
	var/news_message
	var/news_source = "SCP Foundation News Network"
	...
	switch(news_report)
		...
		if(STATION_DESTROYED_NUKE)
			news_message = "We would like to reassure all employees that the reports of a Chaos Insurgency \
		...
		if(NUKE_MISS)
			news_message = "The Chaos Insurgency have bungled a terrorist attack [decoded_station_name], \
		...
		if(OPERATIVES_KILLED)
			news_message = "Repairs to [decoded_station_name] are underway after an elite \
				Chaos Insurgency death squad was wiped out by the crew."
		// Nuke ops results inconclusive - Crew escaped without the disk, or nukies were left alive, or something
		if(OPERATIVE_SKIRMISH)
			news_message = "A skirmish between security forces and Chaos Insurgency agents aboard [decoded_station_name] \
