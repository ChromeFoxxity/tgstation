...
/datum/controller/subsystem/dynamic/proc/send_intercept()
	...
	. = "<b><i>SCP Foundation Department of Intelligence Threat Advisory, Spinward Sector, TCD [time2text(world.realtime, "DDD, MMM DD")], [CURRENT_STATION_YEAR]:</i></b><hr>"
...
/datum/controller/subsystem/dynamic/proc/generate_advisory_level()
	...
	if(prob(PULSAR_REPORT_CHANCE))
		...
		advisory_string += "Your sector's advisory level is Pulsar Star. A large, unknown electromagnetic field has stormed through nearby surveillance equipment, causing major data loss. Partial data was recovered and showed no credible threats to SCP Foundation assets within the Spinward Sector; however, the Department of Intelligence advises maintaining high alert against potential threats due to the lack of complete data."
		return advisory_string
	...
	switch(round(shown_threat))
		if(0 to 19)
			...
			if (show_core_territory)
				...
				advisory_string += "Your sector's advisory level is Blue Star. At this threat advisory, the risk of attacks on SCP Foundation assets within the sector is minor but cannot be ruled out entirely. Remain vigilant."
			else
				advisory_string += "Advisory Level: <b>Green Star</b></center><BR>"
				advisory_string += "Your sector's advisory level is Green Star. Surveillance information shows no credible threats to SCP Foundation assets within the Spinward Sector at this time. As always, the Department of Intelligence advises maintaining vigilance against potential threats, regardless of a lack of known threats."
		if(20 to 39)
		...
		if(66 to 79)
			...
			advisory_string += "Your sector's advisory level is Red Star. The Department of Intelligence has decrypted Cybersun communications suggesting a high likelihood of attacks on SCP Foundation assets within the Spinward Sector. Stations in the region are advised to remain highly vigilant for signs of enemy activity and to be on high alert."
		if(80 to 99)
...
/proc/reopen_roundstart_suicide_roles()
	...
	if(CONFIG_GET(flag/reopen_roundstart_suicide_roles_command_report))
		if(reopened_jobs.len)
			...
			print_command_report(suicide_command_report, "Central Office Personnel Update")
