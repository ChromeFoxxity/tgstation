...
/datum/round_event/market_crash/announce(fake)
	var/list/poss_reasons = list("the alignment of the moon and the sun",\
		...
		"greatly exaggerated reports of SCP Foundation accountancy personnel being \"laid off\"",\
		"a \"great investment\" into \"non-fungible tokens\" by a \"moron\"",\
		"a number of raids from Tiger Cooperative agents",\
		"supply chain shortages",\
		"the \"SCP Foundation+\" social media network's untimely downfall",\
		"the \"SCP Foundation+\" social media network's unfortunate success",\
		"uhh, bad luck, we guess"
	)
	var/reason = pick(poss_reasons)
	priority_announce("Due to [reason], prices for on-station vendors will be increased for a short period.", "SCP Foundation Accounting Division")
...
/datum/round_event/market_crash/end()
	...
	SSeconomy.price_update()
	SSeconomy.update_vending_prices()
	priority_announce("Prices for on-station vendors have now stabilized.", "SCP Foundation Accounting Division")
