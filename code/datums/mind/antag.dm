...
/datum/mind/proc/give_uplink(silent = FALSE, datum/antagonist/antag_datum)
	...
	if(isnull(uplink_loc))
		...
		if(!silent)
			to_chat(traitor_mob, span_boldnotice("Your Chaos Insurgency Uplink has been cunningly implanted in you, for a small TC fee. Simply trigger the uplink to access it."))
